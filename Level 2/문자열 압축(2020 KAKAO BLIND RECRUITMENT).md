...
function solution(s) {
    let answer = s
    // 문자열을 자를 단위 i를 1부터 문자열 길이의 반만큼 반복
    for (let i = 1; i <= s.length / 2; i++) {
        let result = ""
        let ref = s.substr(0, i) // 이전 문자열
        let count = 1 // 압축 가능한 횟수
        for (let j = i; j < s.length; j += i) {
            const compare = s.substr(j, i); // 현재 문자열
            if (ref === compare) {
                count++;
            } else {
                result += count === 1 ? ref : count + ref // 압축이 더 이상 불가능하면 결과에 문자열 합산
                ref = compare // 이전 문자열을 현재 문자열로 변경
                count = 1 // 압축 횟수 초기화
            }
        }
        result += count === 1 ? ref : count + ref // 마지막 문자열 처리
        answer = answer.length < result.length ? answer : result
    }
