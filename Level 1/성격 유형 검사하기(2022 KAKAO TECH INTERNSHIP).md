...
function solution(survey, choices) {
    let answer = ''
    const typeMap = { RT: 0, CF: 0, JM: 0, AN: 0 } // 성격 유형과 해당하는 점수를 담은 객체

    for (let i = 0; i < survey.length; i++) {
        const type = survey[i]
        const score = choices[i]
        const isReverse = typeMap[type] === undefined
        // 성격 유형의 역순 key는 존재하지 않으므로 문자열을 뒤집어서 저장
        const typeName = isReverse ? type.split('').reverse().join('') : type
        const currentScore = isReverse ? score - 4 : -score + 4
        typeMap[typeName] += currentScore
    }

    for (let type in typeMap) {
    // 높은 점수를 가지는 성격 유형의 글자를 answer 문자열에 추가
        answer += typeMap[type] >= 0 ? type.charAt(0) : type.charAt(1)
    }
