# [바탕화면 정리(연습 문제)](https://school.programmers.co.kr/learn/courses/30/lessons/161990)
...
function solution(wallpaper) {
    const getPosition = (isHorizon, isForward) => {
        let i = isForward ? 0 : isHorizon ? wallpaper.length - 1 : wallpaper[0].length - 1
        let isExist = direction => isHorizon ? wallpaper[i].includes("#") : wallpaper.some((row) => row[i] === "#")
        while (!isExist(isHorizon)) {
            i += isForward ? 1 : -1
        }
        return i
    }
    return [getPosition(1, 1), getPosition(0, 1), getPosition(1, 0) + 1, getPosition(0, 0) + 1]
}
```
