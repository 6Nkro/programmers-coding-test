# [개인정보 수집 유효기간(2023 KAKAO BLIND RECRUITMENT)](https://school.programmers.co.kr/learn/courses/30/lessons/150370)
...
function solution(today, terms, privacies) {
    ...
    // day값 계산 함수(문제에서 모든 달은 28일까지 있다고 가정)
    const getDays = (year, month, days) => Number(year) * 336 + (Number(month) - 1) * 28 + Number(days)
    // today의 day값 계산
    const todayTodays = getDays(...today.split("."))

    let termsMap = {}
    terms.forEach(term => {
        const [type, days] = term.split(" ")
        // 약관 종류에 따른 유효기간을 일(day) 단위로 계산하여 객체에 추가
        termsMap = {...termsMap, [type] : Number(days * 28)}
    })

    privacies.forEach((privacy, index) => {
        const [date, type] = privacy.split(" ")
        // 개인정보 수집일과 해당하는 유효기간을 더해 유효 day값 계산
        const validDays = getDays(...date.split(".")) + termsMap[type]
        // 오늘의 day값이 유효 day값보다 크다면 개인정보 파기 목록에 추가
        if (todayTodays >= validDays) answer.push(index + 1)
    })
